...
\begin{equation}
	...
	\tag{BP1}\label{eq:BP1}
\end{equation}
The first term on the right, $\partial{}C/\partial{}a^L_j$, measures how fast the cost is changing as a function of the $j$-th output activation. If, for example, $C$ doesn't depend much on a particular output neuron, $j$, then $\delta^L_j$ will be small, which is what we'd expect. The term, $\sigma'(z^L_j)$, measures how fast the activation function $\sigma$ is changing at $z^L_j$.
